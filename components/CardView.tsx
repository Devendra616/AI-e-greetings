...
const CardView: React.FC<Props> = ({ card, details, onReset }) => {
  ...
  const shareLinks = [
    ...
    {
      name: 'X',
      url: `https://twitter.com/intent/tweet?text=${encodeURIComponent(`I just created a bespoke artisanal greeting for ${details.recipientName}!`)}&url=${encodeURIComponent(window.location.href)}`,
    ...
    {
      name: 'WhatsApp',
      url: `https://wa.me/?text=${encodeURIComponent(`I just created a bespoke artisanal greeting for ${details.recipientName}! ${window.location.href}`)}`,
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-12 animate-fade-in printable-card-container overflow-visible">
              ...
              <div className="w-full h-full relative">
                ...
                {isVideoLoading && (
                  <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-rose-50/20 backdrop-blur-sm">
                    ...
                    <p className="mt-4 text-xs font-black uppercase tracking-widest text-rose-600">Loading Media...</p>
                  </div>
                )}
      ...
      <div className="mt-12 flex flex-wrap gap-6 no-print justify-center items-center">
        <button
          onClick={handlePrint}
          className="px-8 py-5 bg-stone-50 border-2 border-stone-200 text-stone-700 rounded-full hover:bg-stone-100 transition-all font-black uppercase tracking-widest text-[10px] shadow-sm flex items-center gap-3 active:scale-95"
        >
        ...
        <button
          ...
          className="px-10 py-5 bg-stone-900 text-white rounded-full hover:bg-black transition-all font-black uppercase tracking-widest text-[10px] shadow-lg flex items-center gap-3 active:scale-95 disabled:opacity-50"
        >
          {isDownloadingPdf ? (
            <svg className="animate-spin h-4 w-4" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
          ) : (
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 13h6m-6 4h6m-6-8h3" /></svg>
          )}
        ...
        <button
          onClick={onReset}
          className="px-12 py-5 bg-stone-100 text-stone-600 border border-stone-200 rounded-full hover:bg-stone-200 transition-all font-black uppercase tracking-widest text-[10px] shadow-sm active:scale-95"
        >
          Craft Another
        </button>
      </div>
